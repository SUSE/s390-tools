...
# version 3 (at your option) as published by the Free Software Foundation.
# 
		...
		given($fmtstr) {
			...
			when (/LDL/) {
				if ($OPT_FORCE) {
					...
					exit_with("$device: Linux disk layout, unable to use without formatting.", 12);
				}
			}
			when (/CDL/) {
...
Options:
    ...
    -Z, --list-zfcp	display usable ZFCP disks (Device, Size, ID, WWPN, LUN, Dump)
